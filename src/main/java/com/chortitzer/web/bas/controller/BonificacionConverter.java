...
@Component
@Scope("view")
@FacesConverter("com.chortitzer.web.bas.controller.BonificacionConverter")
public class BonificacionConverter implements Converter {
    ...
    public String getAsString(FacesContext context, UIComponent component,
            Object value) {
        try {
            return value instanceof TblDalBonificaciones ? ((TblDalBonificaciones) value).getId().toString() : "";
        } catch (Exception ex) {
            LOGGER.error(Thread.currentThread().getStackTrace()[1].getMethodName(), ex);
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error!", ex.getMessage()));
            return null;
